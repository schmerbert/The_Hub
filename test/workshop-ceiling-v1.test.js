...
test('patch bay separates full ceiling catalog from room profiles and presence', async () => {
  ...
  try {
    ...
    world.move({ sessionId: 'life', doorId: 'door.workshop' });
    assert.deepEqual(world.availableTools('life'), CEILING_WIRES.map(wire => wire.name).filter(name => !['move_through_passage', 'operate_passage', 'turn_fixture', 'tend_hearth'].includes(name) && CEILING_WIRES.find(wire => wire.name === name)?.groupId !== 'spotlight'));
