...
test('World Graph seed is exact and idempotent', async () => {
  ...
  try {
    const first = new WorldGraphStore(path);     assert.equal(first.sqlite.prepare('SELECT COUNT(*) AS count FROM world_nodes').get().count, 12); assert.equal(first.sqlite.prepare('SELECT COUNT(*) AS count FROM world_edges').get().count, 10); assert.deepEqual(first.sqlite.prepare('SELECT id FROM world_nodes ORDER BY id').all().map(row => row.id), ['fixture.packed_sand', 'fixture.stone_bench', 'fixture.workshop_clipboard', 'fixture.workshop_kiln', 'fixture.workshop_ledger', 'fixture.workshop_shelves', 'fixture.workshop_workbench', 'object.tin_cup', 'room.center', 'room.workshop', 'station.control_panel', 'station.spec_table']); assert.equal(first.node('object.tin_cup').state_json.includes('unknown_empty'), false); assert.equal(JSON.parse(first.node('object.tin_cup').state_json).contents, 'unspecified'); assert.equal(first.node('station.spec_table').lifecycle, 'retired');
    ...
    const third = new WorldGraphStore(path); assert.equal(third.sqlite.prepare('SELECT COUNT(*) AS count FROM world_nodes').get().count, 12); assert.equal(third.sqlite.prepare('SELECT COUNT(*) AS count FROM world_edges').get().count, 10); third.close();
