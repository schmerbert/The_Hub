...
test('World Graph seed is exact and idempotent', async () => {
  ...
  try {
    ...
    const second = new WorldGraphStore(path, { topologyVersion: 'b1' }); assert.deepEqual(second.availableTools('session-missing'), ['move_through_door', 'move_through_passage', 'inspect_fixture']); assert.equal(second.current('session-a').room_node_id, 'room.center'); second.close();
    const third = new WorldGraphStore(path, { topologyVersion: 'b1' }); assert.equal(third.sqlite.prepare('SELECT COUNT(*) AS count FROM world_nodes').get().count, 22); assert.equal(third.sqlite.prepare('SELECT COUNT(*) AS count FROM world_edges').get().count, 25); third.close();
...
test('World Graph schema rejects invalid and orphan graph rows', async () => {
  ...
  try {
    const world = new WorldGraphStore(path, { topologyVersion: 'b1' }); const now = new Date().toISOString();
...
test('Workshop source admission is additive Wild custody and Home remains empty', async () => {
  ...
  const db = new HubDatabase(operationalPath); const forest = new ForestStore(forestPath); const spine = new SpineStore(spinePath); const world = new WorldGraphStore(worldPath, { topologyVersion: 'b1' }); let wake;
  try {
    ...
    assert.throws(() => verifyForest({ forestPath, operationalPath, spinePath, worldPath, strictBijection: false }), /Wild source bijection/);
    ...
    const retry = forest.ingestWorkshopSource({ sourceKind: 'workshop_read', source, actionReceiptId: action.receiptId, spineRecordId: frame.record_id, requestRecordId: providerRequestId });
    assert.equal(rows.length, 1); assert.equal(rows[0].existing, false); assert.equal(retry[0].existing, true); assert.equal(forest.listWildEntries().length, 1); assert.equal(forest.sqlite.prepare('SELECT COUNT(*) AS count FROM forest_entries').get().count, 0); assert.equal(forest.listWildEntries()[0].bucket, 'workshop_source'); assert.equal(forest.sqlite.prepare('SELECT schema_version FROM forest_metadata WHERE metadata_id=1').get().schema_version, 1); assert.equal(forest.sqlite.prepare('SELECT schema_name, schema_version FROM wild_metadata WHERE metadata_id=1').get().schema_name, 'forest_wild');
...
test('active Workshop read admits its exact span to Wild with action ancestry', async () => {
  ...
  const db = new HubDatabase(operationalPath); const forest = new ForestStore(forestPath); const world = new WorldGraphStore(worldPath, { topologyVersion: 'b1' }); world.ensureLifespan('lifespan-test'); world.move({ sessionId: 'lifespan-test', wakeId: 'wake-test', doorId: 'door.workshop' });
  try {
    const gateway = new WorldActionGateway({ world, forest, workshop: new WorkshopAdapter(process.cwd(), { maxLines: 8, maxBytes: 120000, maxFiles: 30, maxResults: 10 }) });
...
test('room-gated bounded native loop moves, reads, and keeps Home separate', async () => {
  ...
  try {
    ...
    assert.equal(f.hub.world.listLocationEvents(result.body.sessionId).length, 3); assert.equal(f.hub.forest, null); assert.equal(f.hub.db.getSessionHistory(result.body.sessionId).filter(item => item.messageKind === 'tool_result').length, 3);
