...
test('World Graph seed is exact and idempotent', async () => {
  ...
  try {
    const first = new WorldGraphStore(path); assert.equal(first.sqlite.prepare('SELECT COUNT(*) AS count FROM world_nodes').get().count, 5); assert.equal(first.sqlite.prepare('SELECT COUNT(*) AS count FROM world_edges').get().count, 5); assert.deepEqual(first.sqlite.prepare('SELECT id FROM world_nodes ORDER BY id').all().map(row => row.id), ['fixture.packed_sand', 'fixture.stone_bench', 'object.tin_cup', 'room.center', 'room.workshop']); assert.equal(first.node('object.tin_cup').state_json.includes('unknown_empty'), false); assert.equal(JSON.parse(first.node('object.tin_cup').state_json).contents, 'unspecified');
    ...
    const second = new WorldGraphStore(path); assert.deepEqual(second.availableTools('session-missing'), ['move_through_door']); assert.equal(second.current('session-a').room_node_id, 'room.center'); second.close();
    const third = new WorldGraphStore(path); assert.equal(third.sqlite.prepare('SELECT COUNT(*) AS count FROM world_nodes').get().count, 5); assert.equal(third.sqlite.prepare('SELECT COUNT(*) AS count FROM world_edges').get().count, 5); third.close();
...
test('World Graph schema rejects invalid and orphan graph rows', async () => {
  ...
  try {
    ...
    assert.throws(() => world.sqlite.prepare('INSERT INTO world_nodes VALUES(?,?,?,?,?,?,?)').run('bad.node.type', 'unknown', 'bad', '{}', 'standing', 1, now), /CHECK constraint failed/);
    assert.throws(() => world.sqlite.prepare('INSERT INTO world_nodes VALUES(?,?,?,?,?,?,?)').run('bad.lifecycle', 'room', 'bad', '{}', 'unknown', 1, now), /CHECK constraint failed/);
    assert.throws(() => world.sqlite.prepare('INSERT INTO world_nodes VALUES(?,?,?,?,?,?,?)').run('bad.revision', 'room', 'bad', '{}', 'standing', 0, now), /CHECK constraint failed/);
    assert.throws(() => world.sqlite.prepare('INSERT INTO world_edges VALUES(?,?,?,?,?,?,?)').run('bad.edge.type', 'unknown', 'room.center', 'room.workshop', null, null, now), /CHECK constraint failed/);
    assert.throws(() => world.sqlite.prepare('INSERT INTO world_edges VALUES(?,?,?,?,?,?,?)').run('bad.edge.endpoint', 'door', 'room.missing', 'room.workshop', 'bad', 'Bad', now), /FOREIGN KEY constraint failed/);
    assert.throws(() => world.sqlite.prepare('INSERT INTO world_locations VALUES(?,?,?,?,?,?)').run('bad.location.room', 'room.missing', null, 1, now, now), /FOREIGN KEY constraint failed/);
    assert.throws(() => world.sqlite.prepare('INSERT INTO world_locations VALUES(?,?,?,?,?,?)').run('bad.location.revision', 'room.center', null, 0, now, now), /CHECK constraint failed/);
    ...
    assert.throws(() => world.sqlite.prepare('INSERT INTO world_action_receipts VALUES(?,?,?,?,?,?,?,?,?,?,?)').run('bad.action.room', 'session-test', null, 'room.missing', 'move_through_door', '{}', '{}', 'committed', null, null, now), /FOREIGN KEY constraint failed/);
    assert.throws(() => world.sqlite.prepare('INSERT INTO world_action_receipts VALUES(?,?,?,?,?,?,?,?,?,?,?)').run('bad.action.outcome', 'session-test', null, 'room.center', 'move_through_door', '{}', '{}', 'unknown', null, null, now), /CHECK constraint failed/);
    world.close();
  } finally { await rm(dir, { recursive: true, force: true }); }
});
...
test('active Workshop read admits its exact span to Wild with action ancestry', async () => {
  ...
  try {
    const gateway = new WorldActionGateway({ world, forest, workshop: new WorkshopAdapter(process.cwd(), { maxLines: 8, maxBytes: 50000, maxFiles: 30, maxResults: 10 }) });
    const action = gateway.execute({ sessionId: 'lifespan-test', wakeId: 'wake-test', requestRecordId: 'provider-request-test', spineRecordId: 'spine-record-test', intent: { id: 'read-test', type: 'function', function: { name: 'workshop_read', arguments: '{"path":"src/world/graph.js","start_line":1,"line_count":2}' } } });
...
test('room-gated bounded native loop moves, reads, and keeps Home separate', async () => {
  ...
  try {
    ...
    const world = await (await fetch(`${f.base}/api/world`)).json(); assert.deepEqual(world.tools.map(tool => tool.function.name), ['move_through_door', 'workshop_list', 'workshop_read', 'workshop_search']);
