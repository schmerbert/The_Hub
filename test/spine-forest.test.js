...
test('active runtime passes exact two-breath bodies and creates phase-aware presentation links', async () => {
  ...
  try {
    ...
    const hearth = JSON.parse(firstWake.hearth.returnJson); assert.equal(hearth.kind, 'house_hearth_packet'); assert.equal(hearth.silverBulletSlots.count, 10); assert.equal(hearth.silverBulletSlots.occupied.length, 3); assert.equal(hearth.atoms.some(atom => atom.sourceEventId === BLESSING_SOURCE_EVENT_ID), false);
...
test('health exposes bounded Forest custody state and post-response intake failure', async () => {
  ...
  try {
    ...
    const health = await fetch(`http://127.0.0.1:${hub.server.address().port}/api/health?verify=full`); const healthBody = await health.json(); assert.equal(healthBody.ok, false); assert.equal(healthBody.forestActive, true); assert.equal(healthBody.forestCaughtUp, false); assert.equal(healthBody.forestIntegrity, 'error'); assert.equal(healthBody.forestErrorCode, 'forest_integrity_error'); assert.doesNotMatch(JSON.stringify(healthBody), /resident intake|secret/);
  } finally { await new Promise(resolve => hub.server.close(resolve)); hub.close(); await new Promise(resolve => upstream.close(resolve)); await rm(dir, { recursive: true, force: true }); }
});
