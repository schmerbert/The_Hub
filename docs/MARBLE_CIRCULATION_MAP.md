...
5. Autonomous wakes, outside channels, MCP capability doors, and general connector ingress do not yet exist. [`BINDER_WINDOW_V1.md`](specs/BINDER_WINDOW_V1.md) installs one operator-captured frozen Binder projection. [`SPOTLIGHT_OBSERVATORY_V1.md`](specs/SPOTLIGHT_OBSERVATORY_V1.md) installs an enterable production room, bounded observation validator, deterministic replay proposition, and credential-free Robinhood read normalizer, but no runtime observation crossing: the standalone-host transport and all custody routes remain capped. Box refresh/payment actions, live Robinhood delivery, Pipes live knocks, and general connector ingress remain uninstalled.
...
The production package and World room perform no network request merely through presence or movement. World installs the balcony, five fixtures, and a passive bidirectional Center threshold. Crossing appends the ordinary World movement receipt and mounts or removes the room profile; it invokes no Spotlight hand. Absent observation, custody, strategy, approval, or execution bindings still produce exact fail-closed gate results and ordinary action receipts rather than unknown-tool ambiguity. Robinhood is fitted only as an injectable read normalizer for account discovery, portfolio, positions, and quotes. It strips provider identifiers and has no generic-call or mutation path. Codex authentication remains outside this crossing; a separately owned Hub transport/auth binding is still required before live observations can flow.
