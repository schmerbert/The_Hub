...
2. Resident Journal admission and the `home/journal` bucket are implemented. Backpack custody and Silver Bullet forging remain unimplemented.
...
5. Autonomous wakes, outside channels, and general connector ingress do not yet exist. [`BINDER_WINDOW_V1.md`](specs/BINDER_WINDOW_V1.md) installs one operator-captured frozen Binder projection. Spotlight now has an opt-in standalone Robinhood read crossing with normalized observation custody; execution, automatic refresh, Box payment actions, Pipes live knocks, and general connector ingress remain uninstalled.
...
The room performs no network request through presence or movement. World installs the balcony, five fixtures and passive Center threshold; movement retains only its ordinary receipt. The optional live-read crossing is `World-mounted deliberate observe -> standalone Robinhood MCP read allowlist -> identifier-free observation validator -> append-only Spotlight attempt/settlement and observation custody -> Gateway/Result Rack -> host-return Scrub -> Glass/Spine`. Retained list/read never refresh. Only normalized admitted fields may reach the Resident provider; raw MCP prose, account identifiers and credentials cannot follow them. Missing bindings/authentication refuse without substituted fixture data. Financial hands remain capped. The separate Windows DPAPI OAuth file belongs to the host connector, with explicit standalone sign-in and no Codex-token inheritance. Owner: [Spotlight live read v1](specs/SPOTLIGHT_LIVE_READ_V1.md).

Workshop bounded source inspection now carries `truthful_extent/v1`: the source revision, exact current extent, cumulative same-session document coverage, unread spans, and continuation are separate from model recollection. Document source still follows `Workshop -> World inspection/action receipt -> host-return Scrub/Result Rack -> optional exact Wild admission`; outlines and derived presentation metadata do not enter Forest as independent material. `forest:check` inspects the admitted jurisdiction matrix quickly, while full Forest verification closes Source, Scrub, Spine, World, and Forest ancestry.
6. `place.forest` now truthfully projects the installed `forest.resident` terrain. Recursive Forest frames, scale-relative Home/Wild relationships, Mycelium, and cross-Forest portals do not yet exist; the installed Forest remains one configured custody substrate with its current exact Home and Wild intake laws.
