...
| World | `src/world/graph.js`, `src/world/events.js` | Fresh stores bootstrap through the House Hearth extension; journal-bearing drift refuses; startup does not run A2/B1/Hearth upgrades | `world:migrate-a2`, `world:migrate-b1`, `world:migrate-hearth` with backup confirmation | `npm run world:verify` | Prior lifespan origins remain unchanged; topology extensions are event- and hash-bound; no silent repair |
...
| Roots | `src/ledger/roots.js`, stored with Source | Opens the established forward-only boundary; additive empty Result Exhale exposure tables are startup-safe and fabricate no historical rows; fresh operational stores establish an empty boundary through composition | `npm run roots:establish` for existing pre-boundary stores after backup; any future exposure backfill/reclassification is operator-gated | Roots boundary, Forest Exhale, and full suite | Retention never implies respiration; exposure artifacts remain non-Forest-eligible and pre-installation omissions acquire no invented ancestry |
