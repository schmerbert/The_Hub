...
| `src/core/*` re-exports except active `config.js` | Deprecated / active transition | Older imports retained while ownership moved to dedicated modules | All repository callers migrated and a full import scan plus suite prove absence | Existing subsystem tests; add boundary scan before removal |
...
| Awaiting recipe compatibility method in `src/world/recipes.js` | Deprecated | Callers needing terminal result rather than heartbeat start semantics | All consumers use explicit start plus settlement observation | Workshop heartbeat/recipe tests |
