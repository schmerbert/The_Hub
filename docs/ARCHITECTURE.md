...
The autonomous wake companion beside Source owns self-directed schedules, exact-seat witnesses, append-only plan transitions, and wake-origin receipts. `AutonomousWakeController` owns bench timers and duplicate/stale-seat refusal. `HearthWakeController` owns the separate opt-in ordinary clock: it yields to pending bench promises, opens a fresh lifespan at the House, and invokes the normal Hearth ritual. `WakeService` remains the sole provider crossing and intersects mounted World/Forest tools with the reduced autonomous allowlist before presentation and again before dispatch. This preserves one Resident path while fitting authority by origin.
...
Roots is a separate non-respiratory evidence domain in the operational store. It retains House Hearth packets and content-addressed provider reasoning. New Session Scroll assistant rows hold visible speech/action plus a typed reasoning pointer; exact reasoning is materialized only for the active same-wake tool chain when required and is absent from completed historical API context. Roots may point to Forest evidence, but Forest has no ordinary Exhale path from Roots. See [`ROOTS_BOUNDARY_V1.md`](specs/ROOTS_BOUNDARY_V1.md).
...
| Verified ancestry | `src/integrity/`, `src/spine/verified-index.js` | Rebuildable hash-bound checkpoint generations, compatibility selection, Spine prefix/suffix proof, compact Forest-facing custody, and full-audit fallback; canonical stores remain authoritative |
