...
import {
  assertLegacyA1OperationalAdmission, captureExactA2OperationalBoundary, inspectA2Upgrade, inspectB1Upgrade, inspectTopologyUpgrade,
  migrateA2Boundary, migrateB1Boundary, migrateTopologyExtension, rebuildOperationalTablesForBoundary, tableRows,
  validateLegacyOperationalRelations,
} from './topology-version-migrations.js';
import {
  ACTION_RECEIPT_COLUMNS, APPROVAL_COLUMNS, APPROVAL_RECEIPT_COLUMNS, BRIEF_COLUMNS, EDGE_COLUMNS, FIXTURE_RUNTIME_COLUMNS,
  LOCATION_COLUMNS, NODE_COLUMNS, OBJECT_STATE_COLUMNS, PASSAGE_COLUMNS, TIMER_COLUMNS, assertWorldVerified, createWorldEvent, emptyWorldState,
  insertWorldEvent, installWorldEventSchema, readWorldPhysicalProjection, readWorldProjection, reduceWorldEvent, replayWorldEvents, verifyWorldSqlite,
  ...
  constructor(path, { now = () => Date.now(), eventFailureInjector = null, topologyVersion = 'hearth' } = {}) {
    ...
    try {
      ...
      if (hadWorldSchema) {
        ...
        migrateA2Boundary(this, { requireBoundary: true });
        migrateB1Boundary(this, { admittedLegacy: true });
...
`);
  ...
  migrateA2({ backupConfirmed = false } = {}) {
    ...
    return migrateA2Boundary(this);
  }
  _tableRows(table) {
    return tableRows(this, table);
  }
  _assertLegacyA1OperationalAdmission() {
    return assertLegacyA1OperationalAdmission(this);
  }
  _validateLegacyOperationalRelations({ fixtureRuntimes, timers, briefs, approvals, actionReceipts, approvalReceipts }) {
    return validateLegacyOperationalRelations(this, { fixtureRuntimes, timers, briefs, approvals, actionReceipts, approvalReceipts });
  }
  _rebuildOperationalTablesForBoundary() {
    return rebuildOperationalTablesForBoundary(this);
  }
  _captureExactA2OperationalBoundary() {
    return captureExactA2OperationalBoundary(this);
  }
  _migrateA2Boundary({ requireBoundary = false } = {}) {
    return migrateA2Boundary(this, { requireBoundary });
  }
  inspectB1Upgrade() {
    return inspectB1Upgrade(this);
  }
  migrateB1({ backupConfirmed = false } = {}) {
    ...
    return migrateB1Boundary(this, { admittedLegacy: false });
  }
  inspectHearthUpgrade() {
    return inspectTopologyUpgrade(this, 'hearth');
  }
  migrateHearth({ backupConfirmed = false } = {}) {
    return migrateTopologyExtension(this, 'hearth', { backupConfirmed });
  }
  inspectForestUpgrade() {
    return inspectTopologyUpgrade(this, 'forest');
  }
  migrateForest({ backupConfirmed = false } = {}) {
    return migrateTopologyExtension(this, 'forest', { backupConfirmed });
  }
  inspectBinderWindowUpgrade() {
    return inspectTopologyUpgrade(this, 'binderWindow');
  }
  migrateBinderWindow({ backupConfirmed = false } = {}) {
    return migrateTopologyExtension(this, 'binderWindow', { backupConfirmed });
  }
  inspectSpotlightUpgrade() {
    return inspectTopologyUpgrade(this, 'spotlight');
  }
  migrateSpotlight({ backupConfirmed = false } = {}) {
    return migrateTopologyExtension(this, 'spotlight', { backupConfirmed });
  }
  inspectSpotlightDoorUpgrade() {
    return inspectTopologyUpgrade(this, 'spotlightDoor');
  }
  migrateSpotlightDoor({ backupConfirmed = false } = {}) {
    return migrateTopologyExtension(this, 'spotlightDoor', { backupConfirmed });
  }
  seed() {
    const now = NOW();
    this.transaction(() => {
