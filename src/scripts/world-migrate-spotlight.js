...
if (args.has('--help')) {
  console.log(`Usage: npm run world:migrate-spotlight -- [--apply --backup-confirmed]

Without --apply this command inspects the configured World database without changing it.
Applying requires an exact verified Binder Window World and explicit confirmation of a recoverable backup.`);
  process.exit(0);
}

const config = readConfig(process.env);
const world = new WorldGraphStore(config.worldPath, { topologyVersion: 'binder_window' });
try {
  const inspection = world.inspectSpotlightUpgrade();
  if (!args.has('--apply')) {
    ...
    console.log(JSON.stringify({ worldPath: config.worldPath, mode: 'apply', ...world.migrateSpotlight({ backupConfirmed: true }) }, null, 2));
  }
} finally {
  world.close();
}
