...
if (args.has('--help')) {
  console.log(`Usage: npm run world:migrate-spotlight-door -- [--apply --backup-confirmed]

Without --apply this command inspects the configured World database without changing it.
Applying requires an exact verified Spotlight World and explicit confirmation of a recoverable backup.`);
  process.exit(0);
}

const config = readConfig(process.env);
const world = new WorldGraphStore(config.worldPath, { topologyVersion: 'spotlight' });
try {
  const inspection = world.inspectSpotlightDoorUpgrade();
  if (!args.has('--apply')) {
    ...
    console.log(JSON.stringify({ worldPath: config.worldPath, mode: 'apply', ...world.migrateSpotlightDoor({ backupConfirmed: true }) }, null, 2));
  }
} finally {
  world.close();
}
