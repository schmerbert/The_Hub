...
import { inspectWorkshopInstallationUpgrade, migrateWorkshopInstallation, WORKSHOP_INSTALLATION_MIGRATION_COMMAND } from '../rooms/installation-runtime.js';

const args = new Set(process.argv.slice(2));
if (args.has('--help')) {
  console.log(`Usage: ${WORKSHOP_INSTALLATION_MIGRATION_COMMAND}
...
if (!existsSync(config.worldPath)) {
  ...
  try {
    ...
    if (!verification.verified) {
      ...
      const inspection = inspectWorkshopInstallationUpgrade({ sqlite });
